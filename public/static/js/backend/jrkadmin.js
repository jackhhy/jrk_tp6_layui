...
layui.use(["element", "form", "layer", "okUtils", "okTab", "okLayer", "okContextMenu", "okHoliday", "laydate"], function () {
   ...
   var config = okUtils.local("jrkConfig") || jrkConfig || {};
   objOkTab = okTab;
   ...
   $("body").on("click", ".ok-refresh", function () {
      okTab.refresh(this, function (okTab) {
         //刷新之后所处理的事件
         $.post("/admin/index/clearCache",{},function (res) {
   ...
   $("#alertSkin").click(function () {
      okLayer.open("皮肤动画", "/admin/sysset/alertSkin", "50%", "45%", function (layero) {
      }, function () {
      });
   });
   ...
   function lockShowInit(okUtils) {
      ...
      form.on('submit(lockSubmit)', function (data) {
         console.log(data);
         $.post("/admin/login/unlock",{pass:data.field.lock_password},function (res) {
            layer.msg(res.msg, {time:1500,anim: 6,zIndex: 999999991},function () {
                if (res.code==1){
      ...
      $("#lockQuit").click(function () {
         ...
         window.location.replace("/admin/login/logout");  //替换当前页面
      });
   }
