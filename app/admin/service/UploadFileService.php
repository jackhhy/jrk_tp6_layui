...
use app\common\traits\LogicTrait;
 ...
 * 上传文件
 * 例子：   $res = UploadFileService::instance()->setUploadPath('store/product/'.date('Ymd'))->image('file');
 *  添加上传附件到附件表   AttachMent::attachmentAdd($res['name'],$res['size'],$res['type'],$res['dir'],$res['thumb_path'],$res['image_type']);
 */
class UploadFileService
{
    use LogicTrait;
    ...
    public function image($fileName)
    {
        ...
        $info = [];
        try {
            $uploadType = $this->uploadType ?: sysconfig("upload",'upload_type');
            //TODO 没有选择默认使用本地上传
            if (!$uploadType)
                $uploadType = 1;
            ...
            switch ($uploadType) {
                    ...
                    if (is_array($keys)) {
                        foreach ($keys as $key => &$item) {
                            if (is_array($item)) {
                                ...
                                $info = $this->setUploadInfo($info['dir'], 2, $item['key'], self::setHttpType($info['thumb_path']));
                            }
                        }
                    } else return $keys;
                    break;
                case 3 :
                    ...
                    $info = $this->setUploadInfo(self::setHttpType($serverImageInfo['info']['url']), 3, substr(strrchr($serverImageInfo['info']['url'], '/'), 1));
                    break;
                case 4 :
    ...
    {
        $siteUrl = sysconfig('site','site_url') . '/';
    ...
    {
        $uploadType = sysconfig('upload','upload_type');
        //TODO 没有选择默认使用本地上传
            ...
            case 2 :
                ...
                if (is_array($keys)) {
                    foreach ($keys as $key => &$item) {
                        if (is_array($item)) {
                            $info = Qiniu::imageUrl($item['key']);
                            $info['dir'] = self::setHttpType($info['dir']);
