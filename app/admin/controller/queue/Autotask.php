<?php
    ...
    class Autotask extends BaseController
    {
        ...
        {
            $time = time();
            $logDir = app()->getRuntimePath() ."log/".'crontab/';

            if (!is_dir($logDir)) {
                mkdir($logDir, 0755);
            }
            //筛选未过期且未完成的任务
            $crontabList = \app\admin\model\Queues::where('status', '=', 'normal')->order('weigh desc,id desc')->select();
            $execTime = time();
                ...
                try {
                    if ($crontab['type'] == 'url') {
                        if (substr($crontab['content'], 0, 1) == "/") {
                            // 本地项目URL
                            $message = shell_exec('php ' . app()->getRootPath() . 'public/index.php ' . $crontab['content']);
